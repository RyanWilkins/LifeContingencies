...
function hover(svg, path, data, xScale, yScale, mLeft, mTop, theme) {
  ...
  function moved(e) {
    ...
    dot.attr(
      "transform",
      `translate(${xScale(i) + mLeft},${yScale(s.death_chance) + mTop})`
    );
    ...
    dot
      .select("#texttwo")
      .text(
        `Chance of Death By Age: ${parseFloat(s.death_chance * 100).toFixed(
          1
        )}%`
      );
    ...
    xline
      ...
      .attr("y1", yScale(s.death_chance) + mTop + 5)
      .attr("x2", xScale(i) + mLeft)
      .attr("y2", yScale(0) + mTop);

    yline
      .attr("x1", mLeft)
      .attr("y1", yScale(s.death_chance) + mTop)
      .attr("x2", xScale(i) + mLeft - 5)
      .attr("y2", yScale(s.death_chance) + mTop);
  }
...
export const renderGraph = (
  ...
  var svg = d3.select("#perc-graph");

  // console.log(data);

  var height = $("#perc-graph-container").height();
  var width = $("#perc-graph-container").width();
  ...
  const xScale = d3
    .scaleLinear()
    .domain([cur_age_data ? cur_age_data.age : 0,final_age ? final_age.age : 100])
    .range([0, innerWidth]);
  const yScale = d3.scaleLinear().domain([0, 1]).range([innerHeight, 0]);

  var graph = svg
    .selectAll(".graphGroup")
    .data([`${height},${width},${cur_age_data ? cur_age_data.age : "x"}`], (key) => key);
  ...
  svg
    .append("line")
    .attr("id", "highlightxline")
    .attr("stroke", theme.palette.secondary.main);
  svg
    .append("line")
    .attr("id", "highlightyline")
    .attr("stroke", theme.palette.secondary.main);
  ...
  graphEnter
    ...
    .text("Likelihood Of Death Curve");

  var xaxfunc = d3.axisBottom(xScale).ticks(10);;

  const xaxis = svg.selectAll(".graph_xaxis")
  .data([`${height},${width}`], (key) => key);
 ...
 .enter()
  ...
  // x label
  const xlabel = graphEnter.selectAll(".xaxislabel").data([`${height},${width}`], (key) => key);
  ...
  var yaxfunc = d3.axisLeft(yScale).tickFormat(d3.format(".0%")).ticks(10);
  ...
  yaxis
    ...
    .attr("font-family", theme.typography.subtitle2.fontFamily)
    .attr("fill", theme.palette.primary.contrastText)
    .text("Likelihood Of Death By Age")
  ...
  const renderLine = (agedata, color, id, label, labelwidth) => {
    ...
    hlgroup
      ...
      .attr(
        ...
        `translate(${xScale(agedata.age) + margin.left},${
          yScale(agedata.death_chance) + margin.top
        })`
      )
      .raise();
    ...
    hlgroup
      ...
      .attr("y2", yScale(agedata.death_chance) + margin.top + 5);
    ...
    hlgroup
      ...
      .attr(
        ...
        `translate(${xScale(agedata.age) + margin.left - labelwidth / 2},${
          yScale(agedata.death_chance) + margin.top - 30
        })`
      )
      .attr("fill", "white")
  ...
  const avline = average_age
    ? renderLine(
        ...
        "meanline",
        "Average: " + average_age.age.toFixed(0),
        mobile ? 75 : 100
      )
    : null;
  const line25 = perc_25
    ? renderLine(
        perc_25,
        theme.palette.success.main,
        "perc25line",
        "25th Percentile: " + perc_25.age.toFixed(0),
        mobile ? 100 : 150
      )
    : null;
  const line75 = perc_75
    ? renderLine(
        perc_75,
        theme.palette.success.main,
        "perc75line",
        "75th Percentile: " + perc_75.age.toFixed(0),
        mobile ? 100 : 150
      )
    : null;
